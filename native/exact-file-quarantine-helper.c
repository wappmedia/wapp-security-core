...
static const uint32_t K[64] = {
  ...
  0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5b9cca4f,0x76f988da,
...
static void die(const char *m){fprintf(stderr,"wapp-native-exact-file-quarantine: %s\n",m);exit(20);}
...
static int open_parent_path(int base,const char *path){if(!strcmp(path,"."))return dup(base);int fd=open_beneath(base,path,O_PATH|O_DIRECTORY);if(fd<0)die("target parent unavailable or symlinked");return fd;}
static int mkdir_chain(int base,const char *path){int cur=dup(base);if(cur<0)die("directory descriptor duplication failed");if(!strcmp(path,"."))return cur;char copy[PATH_CAP+1];strcpy(copy,path);char *save=NULL,*part=strtok_r(copy,"/",&save);while(part){if(mkdirat(cur,part,0700)<0&&errno!=EEXIST){close(cur);die("quarantine directory create failed");}int next=open_beneath(cur,part,O_PATH|O_DIRECTORY);if(next<0){close(cur);die("quarantine directory unsafe");}struct stat st;if(fstat(next,&st)<0||st.st_uid!=geteuid()||(st.st_mode&0777)!=0700){close(next);close(cur);die("quarantine directory metadata mismatch");}close(cur);cur=next;part=strtok_r(NULL,"/",&save);}return cur;}
...
static void open_context(const char *root,uint64_t expected_dev,uint64_t expected_ino,const char *operation,int create,uint64_t expected_qdev,uint64_t expected_qino){
    root_fd=open_abs(root,O_PATH|O_DIRECTORY);struct stat rst;
    ...
    parent_fd=open_abs(parent_path,O_PATH|O_DIRECTORY);struct stat pst;
    ...
    if(create){if(expected_qdev||expected_qino)die("apply cannot accept preexisting quarantine identity");if(mkdirat(parent_fd,qname,0700)<0)die(errno==EEXIST?"quarantine destination collision":"quarantine root create failed");}
    qroot_fd=open_beneath(parent_fd,qname,O_PATH|O_DIRECTORY);if(qroot_fd<0)die("quarantine root unavailable");struct stat qst;
    ...
    if(create&&mkdirat(qroot_fd,"files",0700)<0)die("quarantine files root create failed");
    files_fd=open_beneath(qroot_fd,"files",O_PATH|O_DIRECTORY);if(files_fd<0)die("quarantine files root unavailable");
}
static void prepare_live(void){for(size_t i=0;i<target_count;i++){Target *t=&targets[i];char pp[PATH_CAP+1];split_path(t->path,pp,t->live_name);t->live_parent=open_parent_path(root_fd,pp);struct stat pst;if(fstat(t->live_parent,&pst)<0||(uint64_t)pst.st_dev!=t->parent_dev||(uint64_t)pst.st_ino!=t->parent_ino||(uint64_t)pst.st_dev!=t->dev)die("target parent identity drift");t->object_fd=open_exact_at(t->live_parent,t->live_name,t,0);if(t->object_fd<0)die("target exact prestate drift");for(size_t j=0;j<i;j++)if(t->dev==targets[j].dev&&t->ino==targets[j].ino)die("duplicate physical target");}}
...
static int restore_one(Target *t){if(!absent_at(t->live_parent,t->live_name))return -1;int fd=open_exact_at(t->quarantine_parent,t->quarantine_name,t,1);if(fd<0)return -1;if(fchmod(fd,t->mode)<0||fsync(fd)<0){close(fd);return -1;}close(fd);if(syscall(SYS_renameat2,t->quarantine_parent,t->quarantine_name,t->live_parent,t->live_name,RENAME_NOREPLACE)<0)return -1;if(fsync(t->quarantine_parent)<0||fsync(t->live_parent)<0)return -1;fd=open_exact_at(t->live_parent,t->live_name,t,0);if(fd<0)return -1;close(fd);return absent_at(t->quarantine_parent,t->quarantine_name)?0:-1;}
static void apply_transaction(const char *manifest_hex,size_t manifest_hex_len,const char *manifest_sha){
    prepare_live();prepare_quarantine();int mf=openat(qroot_fd,"manifest.hex",O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW|O_CLOEXEC,0400);if(mf<0)die("quarantine manifest create failed");write_all(mf,manifest_hex,manifest_hex_len);if(fsync(mf)<0){close(mf);die("manifest fsync failed");}close(mf);journal_fd=openat(qroot_fd,"journal",O_WRONLY|O_CREAT|O_EXCL|O_APPEND|O_NOFOLLOW|O_CLOEXEC,0400);if(journal_fd<0)die("journal create failed");journal("BEGIN",target_count);
    for(size_t i=0;i<target_count;i++){Target *t=&targets[i];struct stat held,named;if(fstat(t->object_fd,&held)<0||fstatat(t->live_parent,t->live_name,&named,AT_SYMLINK_NOFOLLOW)<0||held.st_dev!=named.st_dev||held.st_ino!=named.st_ino)goto compensate;char hash[65];sha_fd(t->object_fd,hash);if(strcmp(hash,t->sha))goto compensate;if(syscall(SYS_renameat2,t->live_parent,t->live_name,t->quarantine_parent,t->quarantine_name,RENAME_NOREPLACE)<0)goto compensate;if(fsync(t->live_parent)<0||fsync(t->quarantine_parent)<0)goto compensate;t->moved=1;int fd=open_beneath(t->quarantine_parent,t->quarantine_name,O_RDONLY|O_NOFOLLOW);if(fd<0||fchmod(fd,0400)<0||fsync(fd)<0){if(fd>=0)close(fd);goto compensate;}close(fd);fd=open_exact_at(t->quarantine_parent,t->quarantine_name,t,1);if(fd<0||!absent_at(t->live_parent,t->live_name)){if(fd>=0)close(fd);goto compensate;}close(fd);journal("MOVED",i);}
    journal("COMMITTED",target_count);struct stat qst;if(fstat(qroot_fd,&qst)<0)partial("quarantine root receipt unavailable");printf("WAPP_EXACT_FILE_STATE_V1\tQUARANTINED_EXACT\t%zu\t%s\t%llu\t%llu\n",target_count,manifest_sha,(unsigned long long)qst.st_dev,(unsigned long long)qst.st_ino);return;
compensate:
    for(size_t j=target_count;j>0;j--){Target *t=&targets[j-1];if(t->moved&&restore_one(t)<0)partial("automatic compensation failed");}
    journal("COMPENSATED",target_count);die("mutation failed; exact compensation verified");
}
static void open_quarantine_targets(void){for(size_t i=0;i<target_count;i++){Target *t=&targets[i];char live_pp[PATH_CAP+1],qpp[PATH_CAP+1];split_path(t->path,live_pp,t->live_name);split_path(t->path,qpp,t->quarantine_name);t->live_parent=open_parent_path(root_fd,live_pp);t->quarantine_parent=open_parent_path(files_fd,qpp);}}
...
static void reconcile_rollback(void){
    ...
    for(size_t i=0;i<target_count;i++){
        Target *t=&targets[i];int live_fd=open_exact_at(t->live_parent,t->live_name,t,0);int q_fd=open_exact_at(t->quarantine_parent,t->quarantine_name,t,1);
        if(live_fd>=0&&q_fd>=0){close(live_fd);close(q_fd);partial("duplicate exact object on both sides");}
        if(live_fd<0&&q_fd<0)partial("target is neither exact original nor exact quarantine");
        if(live_fd>=0){close(live_fd);if(!absent_at(t->quarantine_parent,t->quarantine_name))partial("non-exact quarantine collision");t->moved=0;}
...
int main(int argc,char **argv){
    ...
    if(!strcmp(mode,"apply"))apply_transaction(manifest_hex,strlen(manifest_hex),manifest_sha);else if(!strcmp(mode,"observe-quarantined"))observe_state(1);else if(!strcmp(mode,"rollback"))rollback_transaction();else if(!strcmp(mode,"reconcile-rollback"))reconcile_rollback();else observe_state(0);cleanup();return 0;
}
#endif
