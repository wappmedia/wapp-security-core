...
assert len(encoded)==policy['launcher_encoded_bytes']==47929 and hashlib.sha256(encoded).hexdigest()==policy['launcher_encoded_sha256']=='9cfe4cba4830d9a648c3bab26e088451ee1629042d035b11306e6b37369477d8'
assert len(binary)==policy['launcher_binary_bytes']==35944 and hashlib.sha256(binary).hexdigest()==policy['launcher_binary_sha256']=='6470e5522d9cddd7b733f3e248ae7aff25c44a3bb9aa52228addfbf62f697dfd'
...
PY
grep -Fq '#define HELPER_BYTES 93304U' "$SOURCE"||fail helper_size_not_compiled
grep -Fq '#define HELPER_SHA256 "f1240b7a873033738c4fb23913706e73531c2be5667ff6b827f1d3c2f7dec047"' "$SOURCE"||fail helper_sha_not_compiled
...
/usr/bin/env -i PATH=/usr/bin:/bin:/usr/sbin:/sbin /bin/bash --noprofile --norc "$probe" "$SITE" "$nonce" f1240b7a873033738c4fb23913706e73531c2be5667ff6b827f1d3c2f7dec047 93304 inventory '' >"$out"
...
if /bin/bash "$probe" "$SITE" "$diagnostic_nonce" f1240b7a873033738c4fb23913706e73531c2be5667ff6b827f1d3c2f7dec047 93304 diagnostic '' >"$TMP/diagnostic.out" 2>"$diagnostic_err";then fail stable_diagnostic_unexpected_success;fi
...
runtime_identity='loader=DEGRADED_ASSURANCE_EPHEMERAL_BOOTSTRAP_V1|launcher_sha=6470e5522d9cddd7b733f3e248ae7aff25c44a3bb9aa52228addfbf62f697dfd|launcher_meta=0:0:700:1:2:35944|helper_sha=f1240b7a873033738c4fb23913706e73531c2be5667ff6b827f1d3c2f7dec047|transport=sealed_memfd_execveat_v1'
...
if /bin/bash "$TMP/bad-probe.sh" "$SITE" "$bad_nonce" f1240b7a873033738c4fb23913706e73531c2be5667ff6b827f1d3c2f7dec047 93304 inventory '' >/dev/null 2>&1;then fail hash_mismatch_accepted;fi
...
if /bin/bash "$probe" "$SITE" "$collision_nonce" f1240b7a873033738c4fb23913706e73531c2be5667ff6b827f1d3c2f7dec047 93304 inventory '' >/dev/null 2>&1;then fail preexisting_directory_accepted;fi
...
if /bin/bash "$probe" "$SITE" "$collision_nonce" f1240b7a873033738c4fb23913706e73531c2be5667ff6b827f1d3c2f7dec047 93304 inventory '' >/dev/null 2>&1;then fail symlink_accepted;fi
