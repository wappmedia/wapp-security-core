...
# process ceiling at a production-valid large snapshot size. Build a real
# provider-neutral tree, then execute the exact release-bound artifact through
# its normal loader so snapshot ordering, runtime identity, helper identity,
# two-pass capture, and serialization are all generated by production code.
VOLATILE_MEMORY_ROOT="$TMP/volatile-memory-root";mkdir "$VOLATILE_MEMORY_ROOT"
cat >"$TMP/create-volatile-memory-tree.c" <<'C'
#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
C
cc -O2 -std=gnu11 -Wall -Wextra -Werror "$TMP/create-volatile-memory-tree.c" -o "$TMP/create-volatile-memory-tree"
"$TMP/create-volatile-memory-tree" "$VOLATILE_MEMORY_ROOT"
VOLATILE_MEMORY_TARGET="p000000-$(printf '%072d' 0)"
VOLATILE_MEMORY_POLICY="C:$(printf %s "$VOLATILE_MEMORY_TARGET"|/usr/bin/xxd -p -c 1000)"
VOLATILE_MEMORY_NONCE="$(printf volatile-memory-contract|/usr/bin/openssl dgst -sha256|/usr/bin/awk '{print $NF}')"
/bin/bash "$TMP/probe" "$VOLATILE_MEMORY_ROOT" "$VOLATILE_MEMORY_NONCE" "$BINARY_SHA" "$BINARY_BYTES" volatile-inventory "$VOLATILE_MEMORY_POLICY" >"$TMP/volatile-memory-output.tsv"
/usr/bin/python3 - "$TMP/volatile-memory-output.tsv" "$BINARY_SHA" <<'PY'
import pathlib,sys
path=pathlib.Path(sys.argv[1]);helper=sys.argv[2];lines=path.read_text(encoding='ascii').splitlines()
assert 64*1024*1024 < path.stat().st_size < 120*1024*1024
assert lines[0].startswith('CAPTURE_NONCE\t')
payload=lines[1:-2];assert payload==sorted(payload)
runtime=[line.split('\t') for line in payload if line.startswith('RUNTIME\t')]
summary=[line.split('\t') for line in payload if line.startswith('SUMMARY\t')]
assert len(runtime)==1 and runtime[0][1]=='PRODUCTION_RELEASE_PINNED_NATIVE_LINUX_X86_64_MEMFD_V1' and runtime[0][3]==helper
assert len(summary)==1 and summary[0][1:9]==['140001','1','140000','140000','0','0','0','0'] and summary[0][9]=='true'
assert lines[-2].startswith('VOLATILE_RUNTIME_CANDIDATE\tBOUNDED_VOLATILE_RUNTIME_CANDIDATE_V1\t')
assert lines[-1].endswith('\tCTIME_ONLY\tOBSERVED_STABLE')
PY
