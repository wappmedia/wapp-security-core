...
actions=[
 {'order':1,'primitive':'QUARANTINE_EXACT_FILE','stage':'EXECUTABLE','target':{'path':'/home/user_42/app_42/wp-content/malicious file.php','parent_device':'1','parent_inode':'2','device':'1','inode':'3','mode':'0644','file_type':'REGULAR'},'before':{'sha256':one,'bytes':10},'after':{'sha256':zero,'bytes':0},'rollback':{'artifact':ref('rollback.json'),'restores':'EXACT_ORIGINAL','automatic':True}},
 {'order':2,'primitive':'REPLACE_EXACT_FILE','stage':'CONFIG','target':{'path':'/home/user_42/app_42/wp-content/.user.ini','parent_device':'1','parent_inode':'2','device':'1','inode':'4','mode':'0644','file_type':'REGULAR'},'before':{'sha256':two,'bytes':20},'after':{'sha256':zero,'bytes':0},'rollback':{'artifact':ref('rollback.json'),'restores':'EXACT_ORIGINAL','automatic':True}},
...
for index,item in enumerate(dispatch,1): lines.append(f'{stamp}\tDISPATCH_VERIFIED order={index} consumer={item["consumer"]} result_sha256={str(index%10)*64}')
