...
actions=[
 ...
 {'order':2,'primitive':'REPLACE_EXACT_FILE','stage':'CONFIG','target':{'path':'/home/user_42/app_42/wp-content/.user.ini','parent_device':'1','parent_inode':'2','device':'1','inode':'4','mode':'0644','uid':'100042','gid':'100042','file_type':'REGULAR'},'before':{'sha256':two,'bytes':20},'after':{'sha256':zero,'bytes':0},'rollback':{'artifact':ref('rollback.json'),'restores':'EXACT_ORIGINAL','automatic':True}},
...
for index,action in enumerate(source['actions'],1):
    ...
    if action['primitive']=='QUARANTINE_EXACT_FILE':
        target=action['target'];relative=target['path'][len(source['site']['root'])+1:]
        target_raw=('\t'.join((relative.encode().hex(),action['before']['sha256'],str(action['before']['bytes']),str(int(target['mode'],8)),target['uid'],target['gid'],target['device'],target['inode']))+'\n').encode()
        parent_raw=f"{target['parent_device']}\t{target['parent_inode']}\n".encode();product=json.load(open(source['product']['seal']['path']));components={item['path']:item['sha256'] for item in product['components']}
        dump(binding,{'tool':'wapp-security-bounded-exact-file-binding','schema':1,'state':'PREPARED_NO_MUTATION','domain':domain,'canonical_root':source['site']['root'],'operation_id':source['operation_id'],'package_sha256':source_sha,'action_orders':[index],'target_count':1,'target_manifest_sha256':hashlib.sha256(target_raw).hexdigest(),'parent_manifest_sha256':hashlib.sha256(parent_raw).hexdigest(),'helper_artifact_sha256':components['libexec/wapp-native-exact-file-quarantine-linux-x86_64.b64.txt'],'launcher_artifact_sha256':components['libexec/wapp-native-exact-file-quarantine-ephemeral-memfd-launcher-linux-x86_64.b64.txt'],'loader_sha256':components['lib/native-exact-file-quarantine-ephemeral-loader.sh'],'authority':False})
    else: dump(binding,{'tool':'synthetic-bounded-dispatch-binding','schema':1,'operation_id':source['operation_id'],'action_order':index,'primitive':action['primitive'],'authority':False})
    dispatch.append({'order':index,'stage':action['stage'],'consumer':consumer_for[action['primitive']],'action_count':1,'action_orders':[index],'binding':ref(binding)})
...
variant duplicate_file_object "v['actions'][1]['primitive']='QUARANTINE_EXACT_FILE';v['actions'][1]['stage']='EXECUTABLE';v['actions'][1]['target']['device']=v['actions'][0]['target']['device'];v['actions'][1]['target']['inode']=v['actions'][0]['target']['inode']";expect_fail duplicate_file_object python3 "$MODEL" verify-package --package "$run/duplicate_file_object.json" --domain "$domain" --now-epoch "$now"
